...
namespace fCraft.ConfigGUI
{
    ...
    {
        private const string MapLoadFilter =
            "Minecraft Maps|*.fcm;*.lvl;*.dat;*.mclevel;*.gz;*.map;*.meta;*.mine;*.save";

        private readonly OpenFileDialog browseTemplateDialog = new OpenFileDialog();

        private readonly BackgroundWorker bwGenerator = new BackgroundWorker();

        private readonly BackgroundWorker bwLoader = new BackgroundWorker();

        private readonly BackgroundWorker bwRenderer = new BackgroundWorker();
...
{5}: {6}";
        ...
        private void LoadGeneratorArgs()
        {
            ...
            nSnowAltitude.Value = generatorArgs.SnowAltitude -
                                  (generatorArgs.CustomWaterLevel ? generatorArgs.WaterLevel : generatorArgs.MapHeight/2);
        ...
        private void SaveGeneratorArgs()
        {
            generatorArgs = new MapGeneratorArgs
            {
                ...
                SnowAltitude =
                    (int) (nSnowAltitude.Value + (xWaterLevel.Checked ? nWaterLevel.Value : nMapHeight.Value/2)),
