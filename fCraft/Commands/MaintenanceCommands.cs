...
namespace fCraft
{
    ...
    internal static class MaintenanceCommands
    {
        ...
        private static void DumpPlayerGroupStats(TextWriter writer, IList<PlayerInfo> infos, string groupName)
        {
            ...
            stat.TopBlockRatio =
                infos.OrderByDescending(info => (info.BlocksBuilt/(double) Math.Max(info.BlocksDeleted, 1))).ToArray();
            ...
            if (infos.Count() > TopPlayersToList*2 + 1)
            {
                foreach (PlayerInfo info in stat.TopBlockRatio.Take(TopPlayersToList))
                {
                    writer.WriteLine("        {0,20:0.000}  {1}",
                        (info.BlocksBuilt/(double) Math.Max(info.BlocksDeleted, 1)), info.Name);
                }
                writer.WriteLine("                           ....");
        ...
        internal static void DoAutoRankAll([NotNull] Player player, [NotNull] PlayerInfo[] list, bool silent,
            string message)
        {
            ...
            sw.Stop();
            player.Message("AutoRankAll: Worked for {0}ms, {1} players promoted, {2} demoted.", sw.ElapsedMilliseconds,
                promoted, demoted);
        }
        ...
        private static readonly CommandDescriptor CdMassRank = new CommandDescriptor
        {
            ...
            IsHidden = true,
            IsConsoleSafe = true,
            Permissions = new[] {Permission.Massrank},
            Help = "",
        ...
        private static void MassRankHandler(Player player, Command cmd)
        {
            ...
            {
                player.Message("&eYou can only use this command from console");
                return;
            }*/
        ...
        private static bool SetPlayerInfoField([NotNull] Player player, [NotNull] string fieldName,
            [NotNull] IClassy info,
        ...
        private static void ShutdownHandler(Player player, Command cmd)
        {
            ...
            else
            {
                ...
                ShutdownParams sp = new ShutdownParams(ShutdownReason.ShuttingDown, delayTime, true, false, reason,
                    player);
                Server.Shutdown(sp, false);
            }
        }
