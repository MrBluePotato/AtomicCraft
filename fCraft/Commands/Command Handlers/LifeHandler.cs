    ...
    public class LifeHandler
    {
        ...
        private static void CreateParams()
        {
            ...
            AddParam(new Param()
            {
                Name = "IntrDelay",
                Help =
                    "&hIf >0 the intermediate state of the life is drawn and shown for this anount of timein msec. If 0 the intermediate state is not shown.",
            ...
            AddParam(new Param()
            {
                Name = "Torus",
                Help =
                    "&hBoolean parameter telling if the life area must be understood as a torus (i.e. the top side is connected to the bottom and the left side with the right one.",
                SetValue = SetTorus
            });
            AddParam(new Param()
            {
                Name = "AutoReset",
                Help =
                    "&hThis parameter tells if the life must be auto reset after the detection of a short-periodical state. Possible values are None (no), ToInitial (i), ToRandom (r).",
        ...
        private static void CreateCommands()
        {
            ...
            AddCommand(new LifeCommand()
            {
                ...
                Help =
                    "&hPrints help on commands. Usage: /life help <command|param>. For the list of parameters type '/life help set'. Commands are: ",
                F = OnHelp
            });
            AddCommand(new LifeCommand()
            {
                ...
                Help =
                    "&hCreates a new life. Usage: /life create <name>. Then mark two blocks to define a *flat* rectangle. The life is created stopped and with default settings. After that you can set params by 'set' command and start it by 'start' command.",
                F = OnCreate
            });
            AddCommand(new LifeCommand()
            {
                ...
                Help =
                    "&hDeletes a life. Usage: /life delete <name>. If this life exists it will be stopped and removed from the map book keeping.",
                F = OnDelete
            });
            AddCommand(new LifeCommand()
            {
                ...
                Help =
                    "&hStarts a stopped life. Usage: /life start <name>. If this life exists and is stopped it will be started. Otherwise nothing happens.",
                F = OnStart
            });
            AddCommand(new LifeCommand()
            {
                ...
                Help =
                    "&hStops a life. Usage: /life stop <name>. If this life exists and is started it will be stopped. Otherwise nothing happens.",
                F = OnStop
            });
            AddCommand(new LifeCommand()
            {
                ...
                Help =
                    "&hSets a life parameter. Usage: /life set <name> <param>=<value>[| <param>=<value>]. Sets parameter 'param' value for the life 'name'. Parameters are: "
                    + _allParams.ToString(),
                F = OnSet
            });
            AddCommand(new LifeCommand()
        ...
        private static void OnHelp(Player p, Command cmd)
        {
            ...
            if (!_commands.TryGetValue(cOrP.ToLower(), out c))
            {
                ...
                {
                    p.Message("&WUnknown life command/parameter " + cOrP +
        ...
        private void LifeCreateCallback(Player player, Vector3I[] marks, object state)
        {
            try
            {
                lock (_world.SyncRoot)
                {
                    ...
                    Life2DZone life = new Life2DZone(_name, _world.Map, marks, player,
        ...
        private static void OnPrint(Player p, Command cmd)
        {
            ...
            p.Message("&y" + l.Name + ": " + (l.Stopped ? "stopped" : "started") + ", delay " + l.Delay +
                      ...
                      ", block types: " + l.Normal + " is normal, " + l.Empty + " is empty, " + l.Dead + " is dead, " +
                      l.Newborn + " is newborn");
        }
