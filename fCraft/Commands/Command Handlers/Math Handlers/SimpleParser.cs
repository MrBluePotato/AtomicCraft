...
namespace fCraft
{
    ...
    {
        private static Dictionary<string, FuncData> _functions;
        private static Dictionary<char, FuncData> _operators; //binary ops
        private static Dictionary<char, FuncData> _soperators; //special ops
        private static Dictionary<char, FuncData> _uoperators; //unary ops
        private static Dictionary<string, IExpressionElement> _consts;
...
//etc
        ...
        private static void ProcessOperator(FuncData op, Expression e, Stack<FuncData> tmpStack)
        {
            while (tmpStack.Count > 0) //while operators with higher or equal precedence
            {
                ...
                if (fd.SpecialKind == SpecialOperandKind.NotSpecial && !fd.IsFunctionOrUnary &&
                    fd.Precedence >= op.Precedence)
                    e.Append(tmpStack.Pop().Func);
                else
                    break;
            }
        ...
        private static string ReadTerm(string s, ref int pos)
        {
            ...
            char ch = s[pos];
            if (_operators.ContainsKey(ch) || _soperators.ContainsKey(ch) || _uoperators.ContainsKey(ch) || /*tmp*/
                ch == '=')
            {
