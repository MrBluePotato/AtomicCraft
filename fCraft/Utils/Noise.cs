...
namespace fCraft
{
    ...
    public sealed class Noise
    {
        ...
        public float InterpolatedNoise(float x, float y)
        {
            ...
            switch (InterpolationMode)
            {
                case NoiseInterpolationMode.Linear:
                    ...
                    return InterpolateLinear(InterpolateLinear(p00, p10, xFloat), InterpolateLinear(p01, p11, xFloat),
                        yFloat);
                ...
                case NoiseInterpolationMode.Cosine:
                    ...
                    p10 = StaticNoise(xInt + 1, yInt);
                    p11 = StaticNoise(xInt + 1, yInt + 1);
                    return InterpolateCosine(InterpolateCosine(p00, p10, xFloat), InterpolateCosine(p01, p11, xFloat),
                        yFloat);
            ...
            switch (InterpolationMode)
            {
                case NoiseInterpolationMode.Linear:
                    ...
                    return InterpolateLinear(
                        InterpolateLinear(InterpolateLinear(p000, p100, xFloat), InterpolateLinear(p010, p110, xFloat),
                            yFloat),
                        InterpolateLinear(InterpolateLinear(p001, p101, xFloat), InterpolateLinear(p011, p111, xFloat),
                            yFloat),
                        zFloat);
                ...
                case NoiseInterpolationMode.Cosine:
                    ...
                    return InterpolateCosine(
                        InterpolateCosine(InterpolateCosine(p000, p100, xFloat), InterpolateCosine(p010, p110, xFloat),
                            yFloat),
                        InterpolateCosine(InterpolateCosine(p001, p101, xFloat), InterpolateCosine(p011, p111, xFloat),
                            yFloat),
                        zFloat);
                    /*
        ...
        public static void ApplyBias([NotNull] float[,] data, float c00, float c01, float c10, float c11, float midpoint)
        {
            ...
            for (int x = offsetX - 1; x >= 0; x--)
            {
                for (int y = offsetY - 1; y >= 0; y--)
                {
                    ...
                    data[x + offsetX, y] += InterpolateCosine((c00 + c10)/2, midpoint, c10, (c11 + c10)/2, x*maxX,
                        y*maxY);
                    data[x, y + offsetY] += InterpolateCosine((c00 + c01)/2, c01, midpoint, (c01 + c11)/2, x*maxX,
                        y*maxY);
                    data[x + offsetX, y + offsetY] += InterpolateCosine(midpoint, (c01 + c11)/2, (c11 + c10)/2, c11,
                        x*maxX, y*maxY);
                }
            }
        }
                ...
                for (int y = heightmap.GetLength(1) - 1; y >= 0; y--)
                {
                    ...
                    else
                    {
                        output[x, y] = (heightmap[x - 1, y - 1]*2 + heightmap[x - 1, y]*3 + heightmap[x - 1, y + 1]*2 +
                                        ...
                                        heightmap[x + 1, y - 1]*2 + heightmap[x + 1, y]*3 + heightmap[x + 1, y + 1]*2)*
                                       BoxBlurDivisor;
                    }
                }
            }
        ...
        public static float[,] GaussianBlur5X5([NotNull] float[,] heightmap)
        {
            ...
            for (int x = heightmap.GetLength(0) - 1; x >= 0; x--)
            {
                for (int y = heightmap.GetLength(1) - 1; y >= 0; y--)
                {
                    ...
                    {
                        output[x, y] = (heightmap[x - 2, y - 2] + heightmap[x - 1, y - 2]*4 + heightmap[x, y - 2]*7 +
                                        heightmap[x + 1, y - 2]*4 + heightmap[x + 2, y - 2] +
                                        heightmap[x - 1, y - 1]*4 + heightmap[x - 1, y - 1]*16 + heightmap[x, y - 1]*26 +
                                        heightmap[x + 1, y - 1]*16 + heightmap[x + 2, y - 1]*4 +
                                        heightmap[x - 2, y]*7 + heightmap[x - 1, y]*26 + heightmap[x, y]*41 +
                                        heightmap[x + 1, y]*26 + heightmap[x + 2, y]*7 +
                                        heightmap[x - 2, y + 1]*4 + heightmap[x - 1, y + 1]*16 + heightmap[x, y + 1]*26 +
                                        heightmap[x + 1, y + 1]*16 + heightmap[x + 2, y + 1]*4 +
                                        heightmap[x - 2, y + 2] + heightmap[x - 1, y + 2]*4 + heightmap[x, y + 2]*7 +
                                        heightmap[x + 1, y + 2]*4 + heightmap[x + 2, y + 2])*GaussianBlurDivisor;
                    }
                }
            }
            return output;
        ...
        public static unsafe void CalculateNormalizationParams(float[,,] map, out float multiplier, out float constant,
        ...
        public static unsafe void Normalize(float[,] map, float low, float high)
        {
