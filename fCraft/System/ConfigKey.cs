...
official server list (if server is public).",
            MinLength = 1, MaxLength = 64)] ServerName,

        [StringKey(ConfigSection.General, "CustomChat",
            @"The name of the custom chat channel",
            MinLength = 1, MaxLength = 12)] CustomChatName,

        [StringKey(ConfigSection.General, "cc",
            @"The name of the custom chat alias",
            MinLength = 1, MaxLength = 12)] CustomAliasName,

        [StringKey(ConfigSection.General, "&cBLOCK",
            @"The word which swears will be replaced with",
            MinLength = 1, MaxLength = 12)] SwearName,


        [StringKey(ConfigSection.General, "Welcome to the server! +hax",
...
Note that having more people on a world increases everyone's bandwidth use.",
            MinValue = 1, MaxValue = 128)] MaxPlayersPerWorld,
...
many powers until they prove themselves trustworthy.")] DefaultRank,
...
changes if your computer's IP or server's port change.")] IsPublic,
...
server's URL will change if you change the port number.",
            MinValue = 1, MaxValue = 65535)] Port,
...
overwhelming the Internet connection with data.",
            MinValue = 1, MaxValue = short.MaxValue)] UploadBandwidth,
        ...
        [BoolKey(ConfigSection.Chat, true,
            @"Color player names in chat and in-game based on their rank.")] RankColorsInChat,

        [BoolKey(ConfigSection.Chat, true,
            @"Color world names in chat based on their build and access permissions.")] RankColorsInWorldNames,

        [BoolKey(ConfigSection.Chat, false,
            @"Show 1-character prefixes in chat before player names. This can be
used to set up IRC-style ""+"" and ""@"" prefixes for ops.")] RankPrefixesInChat,

        [BoolKey(ConfigSection.Chat, false,
            @"Show prefixes in the player list. As a side-effect, Minecraft client
will not show custom skins for players with prefixed names.")] RankPrefixesInList,

        [BoolKey(ConfigSection.Chat, true,
            @"Announce players joining or leaving the server in chat.")] ShowConnectionMessages,

        [BoolKey(ConfigSection.Chat, true,
            @"Show messages when players change worlds.")] ShowJoinedWorldMessages,

        [ColorKey(ConfigSection.Chat, Color.SysDefault,
            @"Color of normal system messages.")] SystemMessageColor,

        [ColorKey(ConfigSection.Chat, Color.CustomDefault,
            @"Color of custom chat channel.")] CustomChatColor,

        [ColorKey(ConfigSection.Chat, Color.HelpDefault,
            @"Color of command usage examples in help.")] HelpColor,

        [ColorKey(ConfigSection.Chat, Color.SayDefault,
            @"Color of messages produced by ""/Say"" command.")] SayColor,
...
colorcodes in announcement and rule files.")] AnnouncementColor,

        [ColorKey(ConfigSection.Chat, Color.PMDefault,
            @"Color of private and rank-wide messages.")] PrivateMessageColor,

        [ColorKey(ConfigSection.Chat, Color.MeDefault,
            @"Color of ""/Me"" command messages.")] MeColor,
...
Announcements are shown to all players, one line at a time, in random order.",
            MinValue = 0)] AnnouncementInterval,
...
the build permission for new worlds will default to this rank.")] DefaultBuildRank,

        [StringKey(ConfigSection.Worlds, "maps",
            @"Custom path for storing map files. If you change this value,
make sure to move the map files before starting the server again.")] MapPath,
...
No verification is dangerous, and may allow players to impersonate others.")] VerifyNames,
...
from joining together.",
            MinValue = 0)] MaxConnectionsPerIP,
...
Warning: Unverified players can log in with ANY name - even as you!")] AllowUnverifiedLAN,
...
to other players to check on them, usually while hidden.")] PatrolledRank,
...
Set this to 0 to disable AntiSpam.",
            AlwaysAllowZero = true, MinValue = 2, MaxValue = 64)] AntispamMessageCount,
...
Set this to 0 to disable AntiSpam.",
            AlwaysAllowZero = true, MinValue = 1, MaxValue = 64)] AntispamInterval,
...
Set this to 0 to disable automatic mute (and only leave the warning).",
            MinValue = 0, MaxValue = 86400)] AntispamMuteDuration,
...
before the player is kicked. Set this to 0 to disable automatic kicks.",
            AlwaysAllowZero = true, MinValue = 0, MaxValue = 64)] AntispamMaxWarnings,
...
but will also prevent many legitimate players from joining.")] PaidPlayersOnly,


        [BoolKey(ConfigSection.Security, false,
            @"Require players to specify a reason/memo when banning or unbanning someone.")] RequireBanReason,

        [BoolKey(ConfigSection.Security, false,
            @"Require players to specify a reason/memo when kicking someone.")] RequireKickReason,

        [BoolKey(ConfigSection.Security, false,
            @"Require players to specify a reason/memo when promoting or demoting someone.")] RequireRankChangeReason,


        [BoolKey(ConfigSection.Security, true,
            @"Announce the reason/memo in chat when someone gets kicked/banned/unbanned.")] AnnounceKickAndBanReasons,

        [BoolKey(ConfigSection.Security, true,
            @"Announce promotions and demotions in chat.")] AnnounceRankChanges,

        [BoolKey(ConfigSection.Security, true,
            @"Announce the reason/memo in chat when someone gets promoted or demoted.")] AnnounceRankChangeReasons,
        ...
        [BoolKey(ConfigSection.Security, true,
            @"Automatically enabled BlockDB on all worlds editable by a certain rank or below.")] BlockDBAutoEnable,

        [RankKey(ConfigSection.Security, RankKeyAttribute.BlankValueMeaning.DefaultRank,
            @"Worlds editable by players of this rank (or below) will have BlockDB enabled automatically.
Has no effect until BlockDBAutoEnable key is set.")] BlockDBAutoEnableRank,
...
A higher setting (120+ seconds) is recommended for busy servers with many maps.",
            AlwaysAllowZero = true, MinValue = 10)] SaveInterval,

        [BoolKey(ConfigSection.SavingAndBackup, false,
            @"Whether to create a backup of every map when the server starts.")] BackupOnStartup,

        [BoolKey(ConfigSection.SavingAndBackup, false,
            @"Create backups any time a player joins a map.
Both a timestamp and player's name are included in the filename.")] BackupOnJoin,

        [BoolKey(ConfigSection.SavingAndBackup, true,
            @"Only create backups if the map was modified since last backup.")] BackupOnlyWhenChanged,
...
Set to 0 to disable periodic backups.",
            MinValue = 0)] DefaultBackupInterval,
...
If exceeded, oldest backups will be deleted first.",
            MinValue = 0)] MaxBackups,

        [IntKey(ConfigSection.SavingAndBackup, 0,
            @"Maximum combined filesize of all backups, in MB.
If exceeded, oldest backups will be deleted first.",
            MinValue = 0)] MaxBackupSize,

        [BoolKey(ConfigSection.SavingAndBackup, true,
            @"Create backups of server data (PlayerDB and IPBanList) on startup.")] BackupDataOnStartup,
        ...
        [EnumKey(ConfigSection.Logging, LogSplittingType.OneFile,
            @"The way log files are organized.")] LogMode,
...
Note that encrypted IRC (via SSL) is not supported.")] IRCBotEnabled,
...
an underscore (_) to the name and retry.",
            MinLength = 1, MaxLength = 32)] IRCBotNick,

        [StringKey(ConfigSection.IRC, "irc.esper.net",
            @"Host or address of the IRC network.")] IRCBotNetwork,

        [IntKey(ConfigSection.IRC, 6667,
            @"Port number of the IRC network. Most networks use port 6667.",
            MinValue = 1, MaxValue = 65535)] IRCBotPort,

        [StringKey(ConfigSection.IRC, "#changeme",
            @"Comma-separated list of channels to join. Channel names should include the hash (#).
One some IRC networks, channel names are case-sensitive.")] IRCBotChannels,

        [BoolKey(ConfigSection.IRC, false,
            @"If checked, all chat messages on IRC are shown in the game.
Otherwise, only IRC messages starting with a hash (#) will be relayed.")] IRCBotForwardFromServer,

        [BoolKey(ConfigSection.IRC, false,
            @"If checked, all chat messages from the server are shown on IRC.
Otherwise, only chat messages starting with a hash (#) will be relayed.")] IRCBotForwardFromIRC,

        [BoolKey(ConfigSection.IRC, false,
            @"Show a message on IRC when someone joins or leaves the Minecraft server.")] IRCBotAnnounceServerJoins,

        [BoolKey(ConfigSection.IRC, false,
            @"Show a message in-game when someone joins or leaves the IRC channel.")] IRCBotAnnounceIRCJoins,

        [BoolKey(ConfigSection.IRC, true,
            @"Announce server events (kicks, bans, promotions, demotions) on IRC.")] IRCBotAnnounceServerEvents,

        [BoolKey(ConfigSection.IRC, false,
            @"Check this if bot's nickname is registered
or requires identification/authentication.")] IRCRegisteredNick,

        [StringKey(ConfigSection.IRC, "NickServ",
            @"Name of the nickname registration service bot
(""Q"" on QuakeNet, ""NickServ"" on most others).",
            MinLength = 1, MaxLength = 32)] IRCNickServ,

        [StringKey(ConfigSection.IRC, "IDENTIFY passwordGoesHere",
            @"Message to send to nickname registration service bot.")] IRCNickServMessage,

        [ColorKey(ConfigSection.IRC, Color.IRCDefault,
            @"Color of IRC messages and event announcements, as seen on the server/in-game.")] IRCMessageColor,

        [BoolKey(ConfigSection.IRC, true,
            @"Whether the bots should use colors and formatting on IRC.")] IRCUseColor,
...
have strict message rate limits, so a delay of at least 500ms is recommended.",
            MinValue = 0, MaxValue = 10000)] IRCDelay,
...
with IRC bots falling behind on messages.",
            MinValue = 1, MaxValue = 3)] IRCThreads,
...
Reports are confidential and are not displayed publicly.")] SubmitCrashReports,

        [EnumKey(ConfigSection.Advanced, fCraft.UpdaterMode.Prompt,
            @"AtomicCraft can automatically update to latest stable versions.
If enabled, the update check is done on-startup.")] UpdaterMode,

        [EnumKey(ConfigSection.Advanced, fCraft.Game.StartMode.None,
            @"AtomicCraft can automatically start a game mode when the server launches.
If enabled, the selected game will automatically start.")] StartMode,

        [StringKey(ConfigSection.Advanced, "",
            @"Command to execute (via operating system's shell) before the update is applied.")] RunBeforeUpdate,

        [StringKey(ConfigSection.Advanced, "",
            @"Command to execute (via operating system's shell) after the update is applied.")] RunAfterUpdate,

        [BoolKey(ConfigSection.Advanced, true,
            @"It is recommended to save a backup of all data files before updating.
This setting allows the updater to do that for you.")] BackupBeforeUpdate,

        [BoolKey(ConfigSection.Advanced, false,
            @"When a player places or deletes a block, vanilla Minecraft server
relays the action back. This is not needed, and only wastes bandwidth.")] RelayAllBlockUpdates,
...
You may use this option to disable the relative updates.")] NoPartialPositionUpdates,
...
Setting this above ""Normal"" may slow down other software on your machine.")] ProcessPriority,
...
drawing commands (like cuboid).",
            MinValue = 10)] BlockUpdateThrottling,
...
reduce bandwidth and CPU use, but will add latency to block placement.",
            MinValue = 10, MaxValue = 10000)] TickInterval,
...
low-bandwidth connections.")] LowLatencyMode,
...
Each state increases the maximum potential memory use per-player.",
            MinValue = 1, MaxValue = 100)] MaxUndoStates,
        ...
        [StringKey(ConfigSection.Advanced, "(console)",
            ...
            MinLength = 1, MaxLength = 64)] ConsoleName,

        [BoolKey(ConfigSection.Advanced, false,
            @"Enable autorank (experimental, unsupported, use at your own risk)")] AutoRankEnabled,

        [BoolKey(ConfigSection.Advanced, true,
            @"Enable sending a heartbeat to Classicube.net
If disabled, heartbeat data is written to heartbeatdata.txt.")] HeartbeatEnabled,

        [BoolKey(ConfigSection.Advanced, false,
            @"If enabled, allows changing worlds' environment settings for WoM clients via /Env")] WoMEnableEnvExtensions,
...
AtomicCraft bind to the same IP every time.")] IP,
...
but will reduce bandwidth use.")] BandwidthUseMode,

        [IntKey(ConfigSection.Advanced, 0,
            @"Automatically restarts the server after a given number of seconds.")] RestartInterval
