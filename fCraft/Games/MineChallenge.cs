...
namespace fCraft.Games
{
    public class MineChallenge
    {
        ...
        public static void getOffGrass()
        {
            ...
            {
                if (world.Map.GetBlock(p.Position.X/32, p.Position.Y/32, (short) (p.Position.Z/32 - 1.59375)) !=
                    Block.Grass)
                {
                    p.Message("&8Thanks.");
                ...
                if (Games.MineChallenge.mode == Games.MineChallenge.GameMode.NULL)
                {
                    if (platform.Values.Count > 0)
                    {
                        foreach (Vector3I block in platform.Values)
                        {
                            if (e.Coords == block)
                            {
                                Player.RaisePlayerPlacedBlockEvent(e.Player, world.Map, block,
                                    world.Map.GetBlock(e.Coords), world.Map.GetBlock(e.Coords),
        ...
        public static void scoreCounter()
        {
            ...
            {
                world.Players.Message("&SIt was a tie! Both teams get a point! &9{0} &S- &C{1}", world.blueScore,
                    world.redScore);
                blueRoundsWon++;
                redRoundsWon++;
            }
        ...
        {
            wait(5000);
            world.Players.Message("&SScores so far: &9Blues {0} &S- &CReds {1}", blueRoundsWon.ToString(),
                redRoundsWon.ToString());
            teamNotify();
            wait(5000);
        }
