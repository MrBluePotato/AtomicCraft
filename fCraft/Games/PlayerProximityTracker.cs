    ...
    public class PlayerProximityTracker
    {
        ...
        public void RemovePlayer(Player p)
        {
            ...
            if (null == _players[pos.X, pos.Y] || !_players[pos.X, pos.Y].Remove(p))
                Logger.Log(LogType.Trace,
                    "PlayerProximityTracker.RemovePlayer: Player " + p.Name + " is not found at its position");
        }
...
//this is not a fatal error, the player, even when existing at some wrong position will not be returned by the find call looking around this wrong position
                Logger.Log(LogType.Error,
                    "PlayerProximityTracker.MovePlayer: Player " + p.Name + " is not found at its previous position");
            }
            AddPlayer(p, newPos);
        }
        ...
        //may return null
        public IEnumerable<Player> FindPlayersAtDistance(Player p, double distInBlocks,
            Func<Player, Player, bool> takePair)
        {
