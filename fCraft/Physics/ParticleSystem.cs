...
namespace fCraft
{
    public interface IParticleBehavior
    {
        ...
        //false if stopped
        bool VisitBlock(World world, Vector3I pos, Block block, Player owner, ref int restDistance,
            IList<BlockUpdate> updates, Block Sending);

        void HitPlayer(World world, Vector3I pos, Player hitted, Player by, ref int restDistance,
    ...
    public class FireworkParticle : PhysicsTask
    {
        ...
        protected override int PerformInternal()
        {
            ...
            if (_first)
            {
                ...
                _world.Map.QueueUpdate(new BlockUpdate(null, (short) _startingPos.X, (short) _startingPos.Y,
                    (short) _nextZ, _block));
                _first = false;
                return _rand.Next(100, 401);
            }
            _world.Map.QueueUpdate(new BlockUpdate(null, (short) _startingPos.X, (short) _startingPos.Y, (short) _nextZ,
                Block.Air));
            Count++;
            _nextZ--;
            if (_world.Map.GetBlock(_startingPos.X, _startingPos.Y, _nextZ) != Block.Air ||
    ...
    public class Particle : PhysicsTask
    {
        ...
        /// <param name="behavior">
        ///     The partcle behavior. Includes how fare and fast it moves, what happens if it hits a player an
        ///     obstacle etc.
        /// </param>
        ...
        private void CheckHitPlayers(List<BlockUpdate> updates)
        {
            ...
            {
                if (ReferenceEquals(p, _owner) && (_startingPos - _pos).LengthSquared <= 2*2)
                    //do not react on owner within 2 blocks of the starting position
                    continue;
                if (p.CanBeKilled() && p.Position.DistanceSquaredTo(_pos.ToPlayerCoords()) <= 33*33)
                    //less or equal than a block
    ...
    internal class ExplosionParticleBehavior : IParticleBehavior
    {
        ...
        {
            hitted.Kill(world,
                String.Format("{0}&S was shot by {1}", hitted.ClassyName,
