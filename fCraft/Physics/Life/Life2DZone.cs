            ...
            for (int i = _bounds.XMin; i <= _bounds.XMax; ++i)
                for (int j = _bounds.YMin; j <= _bounds.YMax; ++j)
                    for (int k = _bounds.ZMin; k <= _bounds.ZMax; ++k)
                        if (
                            creator.CanPlace(_map, new Vector3I(i, j, k), DefaultBlocks[EmptyIdx],
                                BlockChangeContext.Physics) != CanPlaceResult.Allowed)
                            throw new ArgumentException(
                                "This life intersects with prohibited zones/blocks. Creation denied.");
        }
            ...
            lock (_life2d)
            {
                switch (_state)
                {
                    ...
                    case State.FinalizedStep:
                        _state = _life2d.FinalizeStep()
                            ? State.HalfStep
                            : (_autoReset != AutoResetMethod.None ? State.Resetting : State.Stopped);
