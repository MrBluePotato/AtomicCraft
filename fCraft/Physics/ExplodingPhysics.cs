    ...
    public class TNTTask : PhysicsTask
    {
                    ...
                    case Stage.Waiting:
                        if (!_gun)
                        {
                            if (!_world.tntPhysics)
                                // || _map.GetBlock(_pos) != Block.TNT) //TNT was removed for some reason, forget the xplosion
        ...
        private void CreateParticles()
        {
            ...
            for (int i = 0; i < n; ++i)
            {
                ...
                Vector3F direction =
                    (new Vector3F((float) (Math.Cos(phi)*Math.Cos(ksi)), (float) (Math.Sin(phi)*Math.Cos(ksi)),
                        (float) Math.Sin(ksi))).Normalize();
                if (_owner.CanPlace(_map, (_pos + 2*direction).Round(), Block.Obsidian, BlockChangeContext.Manual) ==
                    CanPlaceResult.Allowed)
                {
                    if (_map.GetBlock((_pos + 2*direction).Round()) != Block.Obsidian)
                    {
                        _world.AddPhysicsTask(
                            new Particle(_world, (_pos + 2*direction).Round(), direction, _owner, Block.Obsidian,
                                _particleBehavior), 0);
                    }
                }
            }
            ...
            if (++_currentR <= R)
            {
                ...
                for (int z = _currentR; z >= 0; --z)
                {
                    ...
                    for (double x = 0; x <= Math.Round(r); ++x)
                    {
                        ...
                        for (int iy = ix > rPrev ? 0 : (int) Math.Round(y);
                            iy < Math.Max(Math.Round(y) + 1, Math.Round(yPrev));
                            ++iy)
                        {
                            for (int mx = -1; mx < 2; mx += 2)
                            {
                                for (int my = -1; my < 2; my += 2)
                                {
                                    ...
                                    {
                                        if (
                                            _owner.CanPlace(_map,
                                                new Vector3I(mx*ix + _pos.X, my*iy + _pos.Y, mz*z + _pos.Z), Block.Plank,
                ...
                {
                    if (_owner.CanPlace(_map, new Vector3I(pt.X, pt.Y, pt.Z), Block.Plank, BlockChangeContext.Manual) ==
                        CanPlaceResult.Allowed)
                    {
                        if (_map.GetBlock(pt.X, pt.Y, pt.Z) != Block.Lava)
                            //if another tnt hasnt already caused lava here
                        {
                            UpdateMap(new BlockUpdate(null, (short) pt.X, (short) pt.Y, (short) pt.Z, Block.Lava));
                            foreach (Player p in _world.Players)
                            {
                                if (p.CanBeKilled() &&
                                    p.Position.DistanceSquaredTo((new Vector3I(pt.X, pt.Y, pt.Z)).ToPlayerCoords()) <=
                                    64*64) //less or equal than 2 blocks
        ...
        private void TryAddPoint(int x, int y, int z)
        {
            ...
            if (Block.Lava != prevBlock)
            {
                ...
                {
                    _world.AddPhysicsTask(new TNTTask(_world, new Vector3I(x, y, z), _owner, false, _particles),
                        _r.Next(150, 300));
        ...
        public void HitPlayer(World world, Player hitted, Player by)
        {
            ...
            hitted.Kill(world,
                String.Format("{0}&S was torn to pieces by {1}", hitted.ClassyName,
                    hitted.ClassyName == by.ClassyName ? "theirself" : by.ClassyName));
        }
        ...
        protected override int PerformInternal()
        {
            lock (_world.SyncRoot)
            {
                if (_world.fireworkPhysics)
                {
                    ...
                    {
                        _world.Map.QueueUpdate(new BlockUpdate(null, (short) _pos.X, (short) _pos.Y, (short) (_z - 1),
                            Block.Air));
                        ...
                        for (int X2 = _pos.X - 5; X2 <= _pos.X + 5; X2++)
                        {
                            for (int Y2 = (_pos.Y - 5); Y2 <= (_pos.Y + 5); Y2++)
                            {
                                for (int Z2 = (_z - 5); Z2 <= (_z + 5); Z2++)
                                {
                                    ...
                                    {
                                        _world.AddPhysicsTask(
                                            new FireworkParticle(_world, new Vector3I(X2, Y2, Z2), fBlock),
                                            rand.Next(1, 100));
                                    }
                                }
                            }
                    ...
                    if (_world.Map.GetBlock(_pos.X, _pos.Y, _z - 2) == Block.Lava)
                    {
                        _world.Map.QueueUpdate(new BlockUpdate(null, (short) _pos.X, (short) _pos.Y, (short) (_z - 2),
                            Block.Air));
                    }
                    _z++;
                    _count++;
