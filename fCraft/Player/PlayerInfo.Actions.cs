...
namespace fCraft
{
    public sealed partial class PlayerInfo
    {
        ...
        /// <summary>
        ///     Bans given player and their IP address.
        ...
        ///     Whether AddingIPBan, AddedIPBan,
        ///     BanChanging, and BanChanged events should be raised.
        /// </param>
        /// <exception cref="fCraft.PlayerOpException" />
        public void BanIP([NotNull] Player player, [CanBeNull] string reason, bool announce, bool raiseEvents)
        {
            ...
            lock (actionLock)
            {
                ...
                if (!needIPBan && !needNameBan)
                {
                    ...
                    if (player.Can(Permission.ViewPlayerIPs))
                    {
                        msg = String.Format("Given player ({0}) and their IP address ({1}) are both already banned.",
                            Name, address);
                        colorMsg =
                            String.Format("&SGiven player ({0}&S) and their IP address ({1}) are both already banned.",
                                ClassyName, address);
                    }
                    else
                    {
                ...
                {
                    string msg =
                        String.Format("Given player ({0}) is exempt from IP bans. Remove the exemption and retry.",
                            Name);
                    string colorMsg =
                        String.Format("&SGiven player ({0}&S) is exempt from IP bans. Remove the exemption and retry.",
                            ClassyName);
        ...
        /// <param name="announce"> Whether unban should be publicly announced on the server. </param>
        /// <param name="raiseEvents">
        ///     Whether RemovingIPBan, RemovedIPBan,
        ///     BanChanging, and BanChanged events should be raised.
        /// </param>
        /// <exception cref="fCraft.PlayerOpException" />
        ...
        /// <summary>
        ///     Bans given player, their IP, and all other accounts on IP.
        ///     All players from IP are kicked. Throws PlayerOpException on problems.
        /// </summary>
        /// <param name="player"> Player who is banning. </param>
        /// <param name="reason"> Reason for ban. May be empty, if permitted by server configuration. </param>
        ...
        /// <summary>
        ///     Unbans given player, their IP address, and all other accounts on IP.
            ...
            bool announce, bool raiseEvents, bool auto)
        {
            ...
            if (promoting && !player.Can(Permission.Promote, newRank))
            {
                ...
                string colorMsg =
                    String.Format("&SCannot promote {0}&S to {1}&S: you may only promote players up to rank {2}&S.",
            ...
            {
                string msg =
                    String.Format("Cannot demote {0} (ranked {1}): you may only demote players ranked {2} or below.",
                        Name, Rank.Name,
                        player.Info.Rank.GetLimit(Permission.Demote).Name);
                string colorMsg =
                    String.Format(
                        "&SCannot demote {0}&S (ranked {1}&S): you may only demote players ranked {2}&S or below.",
        ...
        /// <summary>
        ///     Freezes this player (prevents from moving, building, and from using most commands).
        ///     Throws PlayerOpException on problems.
        /// </summary>
        ...
        public void Freeze([NotNull] Player player, bool announce, bool raiseEvents)
        {
            ...
            lock (actionLock)
            {
                ...
                if (IsFrozen)
                {
                    ...
                    string colorMsg = String.Format("&SPlayer {0}&S is already frozen (by {1}&S).", ClassyName,
                        FrozenByClassy);
                    throw new PlayerOpException(player, this, PlayerOpExceptionCode.NoActionNeeded, msg, colorMsg);
                }
        ...
        /// <param name="duration">
        ///     Duration of the mute. If a player is already muted for same or greater length of time,
        ///     PlayerOpException is thrown with NoActionNeeded code. If a player is already muted for a shorter length of time,
        ///     the mute duration is extended.
        /// </param>
