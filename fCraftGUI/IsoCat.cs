...
namespace fCraft.GUI
{
    ...
    public sealed unsafe class IsoCat
    {
        ...
        private readonly int blendDivisor;

        private readonly int dimX;
        private readonly int dimX1;
        private readonly int dimX2;
        private readonly int dimY;
        private readonly int dimY1;
        private readonly int dimY2;
        ...
        private readonly int isoH;
        private readonly int isoOffset, isoX, isoY;
        private readonly int mh34;
                            ...
                            while (z < Map.Height)
                            {
                                ...
                                if (block != 0)
                                {
                                    ...
                                    if (blockUp == 0 || blockLeft == 0 || blockRight == 0 || // air
                                        ...
                                        (block != 20 && (blockUp == 20 || blockLeft == 20 || blockRight == 20)) ||
                                        // glass
        ...
        {
            int pos = (x + (Rot == 1 || Rot == 3 ? offsetY : offsetX))*isoX +
                      (y + (Rot == 1 || Rot == 3 ? offsetX : offsetY))*isoY + z*isoH + isoOffset;
            if (block > 49) return;
        ...
        private byte GetBlock(int xx, int yy, int zz)
        {
            ...
            if (Mode == IsoCatMode.Normal)
            {
                return bp[pos];
                     ...
                     (xx == (Rot == 1 || Rot == 3 ? dimY1 : dimX1) || yy == (Rot == 1 || Rot == 3 ? dimX1 : dimY1) ||
                      zz == Map.Height - 1))
            {
                return 0;
            }
            else if (Mode == IsoCatMode.Cut && xx > (Rot == 1 || Rot == 3 ? dimY2 : dimX2) &&
                     yy > (Rot == 1 || Rot == 3 ? dimX2 : dimY2))
            {
                return 0;
            }
